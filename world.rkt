...
(require racket/random racket/hash "object.rkt" "pos.rkt" "config.rkt")
...
(struct world
  (player actors terrain items width height))

;() -> world
(define (make-world)

  (create-dungeon
   (world (object "player" (pos 10 10) empty) empty empty empty columns rows)
   max-rooms))


;(world dx dy) -> world
(define (world-move-player w dx dy)
  (let ([p (world-player w)])
    (struct-copy world w
      [player (object-setpos 
               p (pos-clamp (pos-delta (object-pos p) dx dy)
...
(define (dig-one-room room [terrain empty])
  (flatten 
   (map (λ (x) (map (λ (y) (object "floor" (pos x y) empty)  ) 
...
(define (dig-horizontal-tunnel x1 x2 y) 
  (let ([xmin (min x1 x2)])
    (map (λ (x) (object "floor" (pos (+ xmin  x) y) empty)) (range (abs (- x1 x2))))))
; (nat nat nat) -> (object ...)
(define (dig-vertical-tunnel y1 y2 x) 
  (let ([ymin (min y1 y2)])
    (map (λ (y) (object "floor" (pos x (+ ymin  y)) empty)) (range (abs (- y1 y2))))))
...
(define (create-dungeon w maxrooms)
  (let ([rooms (create-rooms (world-width w) (world-height w) maxrooms)])
    (struct-copy 
     world w [terrain (append (dig-tunnels rooms) (dig-rooms rooms))])))
